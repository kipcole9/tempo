...
* **It sidesteps the intractability** that motivated Vilain–Kautz — but for a different reason than they did. Tempo's intervals are always *fully grounded* (concrete endpoints, never disjunctive "before-or-after" constraints), so it never enters the NP-hard qualitative-constraint regime. A relation query is a handful of constant-time endpoint comparisons, not constraint propagation. Tempo is an *algebra over concrete values*, not a constraint solver.
...
* Because its values are **fully grounded**, it gets the point algebra's tractability without inheriting the interval algebra's NP-hard constraint reasoning — it isn't a constraint solver.
