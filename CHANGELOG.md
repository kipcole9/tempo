...
* `Tempo.Enumeration.add_implicit_enumeration/1` now raises a clear `ArgumentError` when `Tempo.Iso8601.Unit.implicit_enumerator/2` returns `nil` (e.g. trying to enumerate a fully-specified second-resolution datetime — no finer unit exists). Replaces an opaque `(MatchError) no match of right hand side value: nil`.
...
* Implement `Enumerable.Tempo.Interval`. Closed intervals and open-upper intervals (`1985/..`) now iterate forward one resolution-unit at a time from the `:from` endpoint; fully-open (`../..`) and open-lower (`../1985`) intervals raise `ArgumentError` with a clear message (no anchor from which to iterate). Iteration honours the half-open `[from, to)` convention — the upper bound is exclusive, so adjacent intervals concatenate without overlap or gap. `count/1`, `member?/2`, and `slice/1` currently return `{:error, Enumerable.Tempo.Interval}`; exact implementations are tracked with the set-operations milestone. Previously `Enumerable.Tempo.Interval` was an empty file and every Enumerable call on an interval raised `Protocol.UndefinedError`.
...
* **Per-endpoint IXDTF suffixes on intervals.** `2022-06-15T10:00[Europe/Paris]/2022-06-15T12:00[Europe/London]` now parses; each endpoint may carry its own `[zone_id]`, `[+offset]`, `[u-ca=calendar]`, or `[!critical]` suffix, attached to that endpoint's `%Tempo{}` struct rather than the interval as a whole. Previously the grammar attached the IXDTF suffix only at the top level, so any interval whose first endpoint carried a `[...]` segment failed to parse with `Could not parse … detected at "/…"`. The fix threads `extended_suffix()` through `qualified_endpoint`, validates each embedded segment in `Extended.split_extended/1`, and propagates a critical-suffix error (e.g. `[!UnknownZone]`) as a clear `{:error, "Unknown IANA time zone: …"}`. A top-level IXDTF suffix on an interval (outside both endpoints) propagates to each endpoint that does not already carry its own.
