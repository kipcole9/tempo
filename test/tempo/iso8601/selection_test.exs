defmodule Tempo.Parser.Selection.Test do
  ...
  test "selections" do
    ...
    assert Tempo.from_iso8601("2018Y3ML1K1IN") ==
      {:ok, %Tempo{time: [year: 2018, month: 3, selection: [day_of_week: 1, instance: 1]]}}

    # September 2018, 3rd instance of 08:20
    assert Tempo.from_iso8601("2018Y9MTLT8H20M3IN") ==
      {:ok, %Tempo{time: [year: 2018, month: 9, selection: [hour: 8, minute: 20, instance: 3]]}}
    ...
    assert Tempo.from_iso8601("L5M7K2IN") ==
      {:ok, %Tempo{time: [selection: [month: 5, day_of_week: 7, instance: 2]]}}
    ...
    assert Tempo.from_iso8601("2018YL1K1IN") ==
      {:ok, %Tempo{time: [year: 2018, selection: [day_of_week: 1, instance: 1]]}}
    ...
    assert Tempo.from_iso8601("2018YL{1,2,5}KN") ==
      {:ok, %Tempo{time: [year: 2018, selection: [day_of_week: [1..2, 5]]]}}
    ...
    assert Tempo.from_iso8601("2018Y9ML1K1IN/P5D") ==
      {
        ...
        %Tempo.Interval{
          ...
          from: %Tempo{time: [year: 2018, month: 9, selection: [day_of_week: 1, instance: 1]]},
          to: nil
        }
      }
    ...
    assert Tempo.from_iso8601("2018Y9ML{1,3}K1IN/P5D") ==
      {
        ...
        %Tempo.Interval{
          duration: %Tempo.Duration{time: [day: 5]},
          from: %Tempo{time: [year: 2018, month: 9, selection: [day_of_week: [1, 3], instance: 1]]},
          to: nil
        }
      }
  end
