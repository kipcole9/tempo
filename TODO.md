...
* **Ratify or revert the `V`/`Q` selection designators.** RFC 5545 `BYSETPOS` and `WKST` have no ISO 8601 representation; v0.15.1 invented the project-specific selection designators `V` and `Q` so `Tempo.to_iso8601/1`/`inspect/1` round-trip instead of crashing (see [lib/inspect.ex](lib/inspect.ex) and conformance guide §5). This is now public-ish syntax — anyone who persists a `…V…`/`…Q…` string is committed to it. The alternative was: these features round-trip via the RRULE string only (`Tempo.to_rrule/1`), and `to_iso8601` omits or refuses them. Decide deliberately while it is one release old; reverting later is a breaking change.
...
* **Split the god-module `lib/tempo.ex` (~4400 LOC).** The top-level `Tempo` facade has grown large enough to be a maintainability drag. The natural shape is a thin facade delegating to focused modules (conversion, arithmetic, working-days, recurrence entry points) that mostly exist already. Purely internal — no public-API change — so it can land any time, but it is a real refactor deserving its own session rather than being rushed before a release.

* **Pin the dependency semver story for a 1.0.** Tempo's core deps are themselves pre-1.0: `calendrical ~> 0.10`, `localize ~> 0.44`, `astro ~> 2.3`. A Tempo 1.0 that can break when a `0.x` dependency bumps undercuts the 1.0 stability promise. Decide deliberately: tighten the requirements (e.g. pin minors), coordinate those libraries' own 1.0s, or accept the coupling and document the transitive-breaking-change exposure in the README.

* **Ratify the `V`/`Q` selection designators** — see the note in the v0.15.1 review section above; the 1.0 review re-flags it because 1.0 freezes this as public syntax.
