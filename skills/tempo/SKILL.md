...
| "every Nth weekday / recurring events / occurrences of a rule" | **Recurrence** | `Tempo.RRule.parse!/2` (RFC 5545 string → a recurring `%Tempo.Interval{}`), or `Tempo.Interval.new!(from:, duration:, recurrence: :infinity)` for a simple period; materialise bounded with `Tempo.to_interval(value, bound: …)` |
...
| "given uncertain/relative constraints, are they consistent and what do they imply?" | **Constraint network** | `Tempo.Network.new/0 \|> add_period/3 \|> add_sequence/2 \|> add_relation/4`; solve with `Tempo.Network.Solver.consistent?/1` and `Tempo.Network.Solver.tighten/1` (derives narrowest bounds, even for undated periods) |
