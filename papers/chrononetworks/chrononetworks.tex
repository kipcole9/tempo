...
% from ../time-2027. The class file and its logos (cc-by.pdf,
...
The coverage is \emph{complete} for ChronoLog's boundary vocabulary,
not a selection from it. Every synchronism reduces to one of three
mechanisms. (i)~The qualitative relations include all thirteen of
Allen's --- the four precise endpoint-sharing relations
(\textit{starts}, \textit{started-by}, \textit{finishes},
\textit{finished-by}) alongside containment, overlap, immediate
precedence, and the looser period synchronisms. (ii)~A single
parameterised \emph{boundary} relation compares one endpoint of $A$
with one endpoint of $B$ under any of $\{<, \leq, =, \geq, >\}$; this
one relation form expresses ChronoLog's entire family of ordered
single-boundary synchronisms (``starts/ends strictly or loosely
before/after/at the start/end of'', sixteen in all). (iii)~The metric
\emph{delay} relation states an exact, minimum, or maximum offset
between two endpoints. ChronoLog's \emph{strict composite} synchronisms
(e.g.\ strictly-included-in) are a non-strict relation conjoined with
one strict boundary relation; the only family left outside the layer is
its tolerance and gazetteer-hierarchy relations (\textit{within},
\textit{child-of}, \textit{parent-of}), which are tied to ChronoLog's
period-database features rather than to interval geometry. We validate
the mapping against ChronoLog's own published models
(§\ref{sec:examples}).

\begin{table}[t]
  \caption{Representative ChronoLog relations, their Allen counterpart,
    and their atomic constraints; the boundary and delay rows stand for
    a parameterised family. $s,e$ abbreviate start, end.}
  \label{tab:map}
  \centering
  \begin{tabular}{lll}
    \toprule
...
\paragraph{A native-format corpus.} Beyond the two worked examples, we
exercise the layer against a corpus of published ChronoLog models in the
tool's own save format. ChronoLog's \texttt{.clog} files are JSON; we
decode their periods, sequences, events, and synchronisms directly into
networks and re-solve them. The corpus spans the 26th-dynasty model
above, a New-Kingdom Dynasty~18 model, an Aegean Late-Helladic to
Proto-Geometric ceramic sequence, an Iron~Age Levant model, and a
Mediterranean Late Bronze Age study. Every model decodes and is
consistent. The two that carry historical anchors \emph{inside} the
network --- the 26th dynasty (anchored by the Persian conquest of
525~BCE together with a set of epigraphic Apis-bull delay synchronisms)
and the Mediterranean study (dated events and an Amarna delay) ---
tighten to absolute calendar dates, the former recovering its full reign
chronology exactly. The remaining three are radiocarbon-driven and hence
purely \emph{relative}: their absolute evidence is radiocarbon, which
lies outside the network (§\ref{sec:beyond}), so the layer confirms their
consistency and relative ordering but leaves them unanchored in absolute
time until a historical date is supplied. That a single decoder both
reproduces an anchored chronology to the year and correctly reports a
relative one as floating is itself evidence that ChronoLog's serialised
models and the interval layer express the same object.
...
\paragraph{Scope: two layers.} A chronology has a \emph{relative/metric}
layer --- the order, duration, and synchronism constraints that fix
periods relative to one another and to historical anchors --- and an
\emph{absolute/scientific} layer, in which radiocarbon determinations are
calibrated against a reference curve (IntCal) and combined by Bayesian
modelling (OxCal) into probabilistic calendar ranges.
\texttt{Tempo.Network} is the first layer in full: it derives
\emph{deterministic} bounds from order, duration, and anchors. It is
deliberately not the second --- it neither calibrates radiocarbon nor
computes probability distributions, and ChronoLog's per-period
radiocarbon samples (its \texttt{C14\dots ForExport} fields, destined for
OxCal) are carried through decoding but excluded from the constraint
graph. Uncertainty here is bounds, not probability; this is why a
radiocarbon-only model (§\ref{sec:examples}) is consistent and internally
ordered yet floating in absolute time. The two layers compose --- the
relative network is the natural prior structure for the Bayesian one ---
but the statistical layer belongs to a tool such as OxCal, not to an
interval library.
...
supported. Cross-unit duration conversion (e.g.\ a year-grained
